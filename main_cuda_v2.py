...
def compute_overlap_loss_per_step(p_history, sigma=0.5, time_window=10):
    """
    计算轨迹重叠损失 (逐时间步版本)
    输入 p_history: [Time, Batch, 3]
    输出: [Batch, Time] 的 Loss 矩阵，表示每个时间步的拥挤程度
    """
    # 调整维度: [Time, Batch, Dim] -> [Batch, Time, Dim]
    ...
    # 4. 聚合每个时间步的能量
    # sum(dim=2) 表示对于时刻 t，累加它与其他所有时刻 t' 的重叠能量
    # 结果形状 [Batch, Time]
...
for i in pbar:
    ...
    trajectory_lgn_weights = [] # 用于存储每一步的权重
    h = None
    ...
    # 3. 计算【逐时间步】的原始损失 (Raw Losses per Step)
    # 所有计算都不做 .mean()，保持 [T, B] 的形状
    
    # (1) 速度损失 [T, B]
    loss_speed_seq = F.smooth_l1_loss(
        v_history.norm(2, -1), 
        torch.ones_like(v_history.norm(2, -1)) * 5.0, 
        reduction='none' # 关键：不进行平均
    )
    ...
    # (4) 探索损失 [T, B] (使用修改后的函数)
    # 这里的函数返回 [Batch, Time]，需要转置为 [Time, Batch] 以对齐
    loss_exploration_seq = compute_overlap_loss_per_step(p_history, sigma=1.0, time_window=50).permute(1, 0)

    # (5) 平滑损失 [T, B]
    # 取最后 T 个时间步的差分，以对齐 weights_seq 的长度
    loss_smooth_seq = act_buffer.diff(1, 0)[-args.timesteps:].pow(2).sum(-1)

    # 4. 【逐时间步加权】 (Element-wise Weighted Sum)
    # 形状说明:
    # weights_seq[:, :, 0] -> [T, B]
    # loss_speed_seq       -> [T, B]
    # 它们逐元素相乘，表示 "T时刻的权重" * "T时刻的损失"
    
    # weights对应关系: 0:Speed, 1:Dir, 2:Avoid, 3:Expl, 4:Smooth
    weighted_loss_map = (
    ...
    # --- Meta Loss Components (依然是全局统计指标) ---
    ...
    with torch.no_grad():
        ...
        if is_save_iter(i):
            ...
            p_cpu = p_history[:, 0].cpu()
            ax.plot(p_cpu[:, 0], p_cpu[:, 1])
            ax.set_title(f"Iter {i}")
            writer.add_figure('Trajectory', fig_p, i + 1)
            plt.close(fig_p)
