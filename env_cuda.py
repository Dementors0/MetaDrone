...
class Env:
    ...
    def reset_drone_only(self):
        """Reset drone states while keeping the current obstacle layout unchanged."""
        B = self.batch_size
        device = self.device
        ...
        scale_xyz = torch.cat([
            ...
            rd + 0.5,
            torch.rand_like(scale) - 0.5], -1)
        self.p = self.p_init * scale_xyz + torch.randn_like(scale_xyz) * 0.1
        self.p_target = self.p_end * scale_xyz + torch.randn_like(scale_xyz) * 0.1
