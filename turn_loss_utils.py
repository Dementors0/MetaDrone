...
def _wrap_angle(delta):
    """Wrap angle to [-pi, pi] to avoid branch-cut discontinuity."""
...
def compute_turn_preference_loss_xy_windowed(
    ...
    Cumulative turn definition in a fixed window:
      |sum(dtheta)|, where dtheta = wrap(yaw[t] - yaw[t-1]) is signed.
    This is vector-sum magnitude of turn increments (opposite turns can cancel).
    """
    T, B, _ = v_history.shape
    device = v_history.device
    dtype = v_history.dtype
    ...
    # Window over transition steps: (left[t], t] on prefix axis.
    ...
    # Encourage larger cumulative turning by minimizing this term.
    loss_turn_seq = 1.0 - torch.clamp(mean_turn / torch.pi, 0.0, 1.0)
    # t=0 has no valid transition.
    loss_turn_seq[0] = 0.0
    # If no valid transition in window, keep neutral zero contribution.
