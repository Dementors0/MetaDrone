...
for i in pbar:
    ...
    if train_lgn_phase:
        # === 阶段 1: LGN 优化 (余弦相似度 + 强正则化) ===
        
        # 1. 计算 Proxy 梯度 (create_graph=True 用于 LGN 链式法则)
        # [修改点] 显式 retain_graph=True，防止 grad_proxy 计算后释放 graph
        grad_proxy = torch.autograd.grad(proxy_loss, worknet.parameters(), create_graph=True, retain_graph=True, allow_unused=True)
        
        # 2. 计算 Meta 梯度 (作为目标方向)
        # [修改点] 显式 retain_graph=True，防止 grad_meta 计算后释放 graph (这会导致后续 backward 失败)
        ...
        for gp, gm in zip(grad_proxy, grad_meta):
            if gp is not None and gm is not None:
                ...
                g_meta_list.append(gm.view(-1).detach()) 
        ...
        if len(g_proxy_list) > 0:
            ...
            # 5. 分量级权重正则化 (Element-wise Regularization)
            mean_weights = weights_seq.mean(dim=[0, 1]) # [5]
            loss_reg = (mean_weights - 1.0).pow(2).mean() 
            
            # 6. 熵正则化 (Entropy Regularization)
            probs = weights_seq / (weights_seq.sum(dim=-1, keepdim=True) + 1e-6)
            entropy = - (probs * torch.log(probs + 1e-6)).sum(dim=-1).mean()
            loss_entropy = -entropy 

            # 总损失
            total_lgn_loss = loss_cosine + 0.5 * loss_reg + 0.01 * loss_entropy
    ...
    with torch.no_grad():
        ...
        smooth_dict({
            'Loss_all/Proxy_Total': proxy_loss,
            'Loss_all/Meta_Total': meta_loss,
            'Loss_all/Raw_Speed': loss_speed_seq.mean(),
            'Loss_all/Raw_Dir': loss_direction_seq.mean(),
            'Loss_all/Raw_Avoid': loss_avoidance_seq.mean(),
            'Loss_all/Meta_pos':loss_meta_pos,
            'Loss_all/Meta_coll':loss_meta_coll * 5,
            'Loss_all/Meta_ctrl':loss_meta_ctrl * 0.001,
        ...
        if train_lgn_phase and 'total_lgn_loss' in locals():
             smooth_dict({
                 'Loss_LGN/Cosine_Neg': loss_cosine, 
                 'Loss_LGN/Reg': loss_reg,
                 'Loss_LGN/Entropy_Neg': loss_entropy
             })
